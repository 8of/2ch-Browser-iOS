...
//    _toolbar.barStyle = UIBarStyleBlackTranslucent;
    ...
    if (self.displayNavArrows) {
        ...
        arrowPathFormat = @"%@page.png";
        
        _previousButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:[NSString stringWithFormat:arrowPathFormat, @"Prev"]] style:UIBarButtonItemStylePlain target:self action:@selector(gotoPreviousPage)];
        _nextButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:[NSString stringWithFormat:arrowPathFormat, @"Next"]] style:UIBarButtonItemStylePlain target:self action:@selector(gotoNextPage)];
    }
...
- (void)performLayout {
    ...
    if (_enableGrid) {
        ...
        [items addObject:[[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@.png", buttonName]] style:UIBarButtonItemStylePlain target:self action:@selector(showGridAnimated)]];
    } else {
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
            ...
            if (self.displaySelectionButtons) {
                ...
                [selectedButton setImage:[UIImage imageNamed:@"ImageSelectedOff.png"] forState:UIControlStateNormal];
                [selectedButton setImage:[UIImage imageNamed:@"ImageSelectedOn.png"] forState:UIControlStateSelected];
...
- (void)updateNavigation {
    ...
    if (_gridController) {
        if (_gridController.selectionMode) {
            self.title = NSLocalizedString(@"Выберите фото", nil);
        } else {
            NSString *photosText;
            if (numberOfPhotos == 1) {
                photosText = NSLocalizedString(@"изображение", @"Used in the context: '1 photo'");
            } else {
                photosText = NSLocalizedString(@"изображений", @"Used in the context: '3 photos'");
            }
